	...
	if ( !class_exists('pxl') ) :
		class pxl {
			...
			public  function get_post_by_title($page_title, $post_type = "post" ,$output = OBJECT) {
				global $wpdb;
				$post_request = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type= %s", strtolower($page_title), strtolower($post_type) ));
			...
			public  function get_active_terms( $taxonomy ) {
				global $wpdb, $post_type;
				return $wpdb->get_results(
					$wpdb->prepare(
						"
			...
			public  function loop_items( $args, $params, $template, $loop_fn = false ) {
				global $post;
				$tmp_post = $post;
			...
			public  function timthumb( $image, $args = false, $size = 'medium', $title = '', $echo_url = false ) {
				$defaults = array( 'w' => 100 ,'h' => 100 ,'q' => 100 ,'a' => false ,'zc' => 0 ,'f' => false ,'s' => false ,'cc' => false ,'ct' => false, 'lp' => 'random' );
