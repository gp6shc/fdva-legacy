		...
		class pxlResources {
			...
			function admin() {
				...
				foreach ($register as $ext => $files) {
					foreach ($files as $file => $args)
						self::register( $ext, $file, $args, array(CHILD_PATH,FW_RESOURCES), 'register' );
				} // Register all Files
			...
			function theme() {
				...
				foreach (self::$setup['theme']['resources'] as $ext => $files)
					foreach ($files as $file => $args)
						if( $file != 'admin' ) self::register( $ext, $file, $args );
			}
			...
			private function register( $ext, $file, $args, $dir = false, $action = 'enqueue' ) {
				if ( $ext == 'fonts' ) wp_enqueue_style( $file, "http://fonts.googleapis.com/css?family=$file:$args" );
				else if ( $args ) {
